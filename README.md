...
- 播放/暂停、跨语音块前后 15 秒、上一章/下一章、0.75×–2.5×、定时停止。
...
- 离线语音包：7 个中文离线音色（50–176 MB）点击即下载并安装；已下载的安装文件可重装或手动删除；切换语音包/引擎/说话人不会自动删除任何缓存，朗读缓存按引擎安装时间与说话人纪元精确区分，切回旧音色时缓存仍然有效。
...
booklisten-alpha16.apk  当前推荐安装包
booklisten-alpha14.apk  整章进度条与语音包下载基线版本安装包
```

## 安装包

- `booklisten-alpha16.apk`：当前版本，建议覆盖安装以保留书架和播放进度。
