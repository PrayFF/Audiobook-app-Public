...
- 书架支持手动重命名、删除前二次确认；收藏夹分组（新建/重命名/删除/移入移出）、书籍上移下移排序、列表⇄九宫格排版切换；章节进度条按整章内容计算。
- 网页书籍自动生成全书目录（只存章节标题与链接，正文按需抓取）；目录支持单个/批量改名（去固定字段、去开头/结尾 N 字）与恢复默认章节名。
...
booklisten-alpha19.apk  当前推荐安装包
...
- `booklisten-alpha19.apk`：当前版本，建议覆盖安装以保留书架和播放进度。
